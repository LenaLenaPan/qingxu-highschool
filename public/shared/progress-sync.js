(() => {
  ...
  var resolveCurrentUser = async () => {
    const client = getClient();
    let currentUser3 = client.currentUser();
    if (!currentUser3 && isBrowser2()) {
      try {
        await hydrateSession();
      } catch {
      }
      currentUser3 = client.currentUser();
    }
    if (!currentUser3) throw new AuthError("No user is currently logged in");
    return currentUser3;
  };
  ...
  var updateUser = async (updates) => {
    const currentUser3 = await resolveCurrentUser();
    try {
      const updatedUser = await currentUser3.update(updates);
  ...
  // src/progress-sync.ts
  var CACHE_KEY = "qingxu-cloud-progress-cache-v1";
  var BUCKET_LABELS = {
    today: "\u4ECA\u5929",
    week: "\u672C\u5468",
    later: "\u540E\u7EED",
  ...
  var plan = { subjects: {}, tasks: [] };
  var progress = { version: 1, tasks: {}, updatedAt: null };
  ...
  var saveCache = () => localStorage.setItem(CACHE_KEY, JSON.stringify(progress));
  var taskState = (task) => {
  ...
  var taskStatus = (task, state = taskState(task)) => {
  ...
  async function saveTask(task, state) {
    if (!currentUser2) throw new Error("\u8BF7\u5148\u767B\u5F55\uFF0C\u518D\u4FEE\u6539\u8FDB\u5EA6\u3002");
    ...
    if (!response.ok) {
      if (previous) progress.tasks[task.id] = previous;
      else delete progress.tasks[task.id];
      saveCache();
      renderAll();
  ...
  function legacyUpdates() {
    ...
    let shared = {};
    try {
      shared = JSON.parse(localStorage.getItem("qingxu-learning-v1") || "{}");
    } catch {
    }
    for (const task of plan.tasks) {
      ...
      if (task.id === "math-chapter2-diagnostic") {
        try {
          if (JSON.parse(localStorage.getItem("qingxu-math-ch2-diagnostic-v1") || "{}").submitted) done.add("attempt");
        } catch {
        }
      }
      if (task.id === "chemistry-special-1") {
        try {
          if (JSON.parse(localStorage.getItem("qingxu-chem-special-1-v1") || "{}").locked) done.add("attempt");
        } catch {
        }
      }
  ...
  function renderAuth() {
    ...
    if (loginPanel) loginPanel.toggleAttribute("hidden", !!currentUser2 || !!inviteToken);
    if (accountPanel) accountPanel.toggleAttribute("hidden", !currentUser2);
    if (invitePanel) invitePanel.toggleAttribute("hidden", !inviteToken);
    const email = $("[data-qx-user-email]");
    if (email) email.textContent = currentUser2?.email || "";
    const syncState = $("[data-qx-sync-state]");
    if (syncState) syncState.textContent = currentUser2 ? "\u5DF2\u8FDE\u63A5\u4E91\u7AEF\u8FDB\u5EA6" : "\u767B\u5F55\u540E\u5728\u624B\u673A\u4E0E\u7535\u8111\u95F4\u540C\u6B65";
  }
  function taskCard(task, compact = false) {
    const state = taskState(task);
    const status = taskStatus(task, state);
    const disabled = !currentUser2 || task.contentState === "preparing";
    const link = task.contentState === "ready" && task.url ? `<a class="qx-task-link" href="${escapeHtml(task.url)}">\u6253\u5F00\u5185\u5BB9</a>` : `<span class="qx-preparing">\u5185\u5BB9\u6574\u7406\u4E2D</span>`;
    if (compact) {
      ...
      <div><h3>${escapeHtml(task.title)}</h3><small>${escapeHtml(status.label)} \xB7 ${status.completed}/${task.steps.length} \u6B65</small></div>
      ${link}
    </article>`;
    }
    ...
    <div class="qx-task-foot">${link}<label>\u5B89\u6392\u5230 <select data-bucket-task="${task.id}" ${!currentUser2 ? "disabled" : ""}>${["today", "week", "later", "done"].map((bucket) => `<option value="${bucket}" ${state.bucket === bucket ? "selected" : ""}>${BUCKET_LABELS[bucket]}</option>`).join("")}</select></label></div>
  </article>`;
  }
  function renderDashboard() {
    ...
    if (date) date.textContent = new Intl.DateTimeFormat("zh-CN", { month: "long", day: "numeric", weekday: "short" }).format(/* @__PURE__ */ new Date());
    const count = $("[data-qx-count]");
    if (count) count.textContent = `${finished}/${tasks.length}`;
    const ring = $("[data-qx-ring]");
  ...
  function renderProgressPage() {
    ...
    $$("[data-task-id]").forEach((input) => input.addEventListener("change", async () => {
      const task = plan.tasks.find((item) => item.id === input.dataset.taskId);
      if (!task) return;
      const state = taskState(task);
      const done = new Set(state.done);
      if (input.checked) done.add(input.dataset.stepId);
      else done.delete(input.dataset.stepId);
      const bucket = done.size === task.steps.length ? "done" : state.bucket === "done" ? "week" : state.bucket;
      try {
        await saveTask(task, { done: [...done], bucket });
      } catch (error) {
        setMessage(error.message, "error");
      }
    }));
    $$("[data-bucket-task]").forEach((select) => select.addEventListener("change", async () => {
      const task = plan.tasks.find((item) => item.id === select.dataset.bucketTask);
      if (!task) return;
      const state = taskState(task);
      try {
        await saveTask(task, { done: state.done, bucket: select.value });
      } catch (error) {
        setMessage(error.message, "error");
      }
    }));
  ...
  async function setupAuth() {
    try {
      ...
      currentUser2 = callback?.user || await getUser();
    } catch (error) {
      setMessage(error.message || "\u767B\u5F55\u670D\u52A1\u6682\u4E0D\u53EF\u7528\u3002", "error");
    }
    renderAuth();
    if (currentUser2) {
      try {
  ...
  function bindAuthForms() {
    $("[data-qx-login-form]")?.addEventListener("submit", async (event) => {
      ...
      const email = ($("[data-qx-email]")?.value || "").trim();
      const password = $("[data-qx-password]")?.value || "";
      setMessage("\u6B63\u5728\u767B\u5F55\u2026");
      try {
        currentUser2 = await login(email, password);
        await loadCloud();
        setMessage("\u8FDB\u5EA6\u5DF2\u4ECE\u4E91\u7AEF\u540C\u6B65\u3002");
        renderAll();
      } catch (error) {
    ...
    $("[data-qx-invite-form]")?.addEventListener("submit", async (event) => {
      event.preventDefault();
      const password = $("[data-qx-invite-password]")?.value || "";
      try {
        currentUser2 = await acceptInvite(inviteToken, password);
        inviteToken = "";
        await loadCloud();
        setMessage("\u8D26\u53F7\u5DF2\u542F\u7528\uFF0C\u8FDB\u5EA6\u540C\u6B65\u5B8C\u6210\u3002");
        renderAll();
      } catch (error) {
        setMessage(error.message || "\u8D26\u53F7\u542F\u7528\u5931\u8D25\u3002", "error");
      }
    });
    $("[data-qx-reset-form]")?.addEventListener("submit", async (event) => {
      event.preventDefault();
      const password = $("[data-qx-reset-password]")?.value || "";
      try {
